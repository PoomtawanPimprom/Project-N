...
import { ManageReportAction, } from "../[id]/action";
...
const InfoReportComponent = async ({ reportId }: prop) => {
  const manageReportActionWithReportId = ManageReportAction.bind(null, reportId);
  ...
  const reportStatus = (await prisma.reportStatus.findMany()) as reportStatusInterface[];
      ...
      <div className="flex flex-col border  bg-gray-50 p-4 rounded-xl shadow-xl space-y-4">
        ...
        <div className=" grid grid-cols-2 w-full gap-2 ">
          ...
          <div className="grid grid-rows-3">
            <div>
              ...
              <p className="font-semibold">{report?.product?.category?.name}</p>
            </div>
            <div>
        ...
        <div className="flex flex-col space-y-1">
          ...
          <div className="flex w-full">
            <form action={manageReportActionWithReportId} className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 space-x-2 w-full">
              <select
